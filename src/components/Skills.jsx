...
export default function Skills() {
  ...
  return (
    <section ref={sectionRef} id="skills" className="bg-[var(--bg-tint)] py-24 px-6">
      <div className="max-w-5xl mx-auto">
        <h2 className="font-display text-4xl md:text-5xl font-bold text-[var(--gold)] mb-12">Skills</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {skillCategories.map((cat, catIdx) => (
            ...
            >
              <h3 className="text-center font-semibold text-[var(--text)] mb-6 text-base">{cat.title}</h3>
                ...
                {cat.skills.map((sk) => {
                  ...
                  return (
                    <span
                      key={sk.name}
                      data-skill-badge={bi}
                      className="inline-flex items-center gap-2 bg-[var(--gold)] text-white px-4 py-2 rounded-full text-sm font-semibold shadow-sm hover:bg-[var(--gold-dark)] transition-colors duration-200 cursor-default"
                      style={{
                        opacity: 0,
